...
class Class {
  constructor(rawDoc = {}, options = {}, ownerDoc) {
    // Set default options.
    _defaults(options, {
    ...
    // attach the owning document, if any
    if(ownerDoc) {
      doc._ownerDoc = ownerDoc;
    }
    ...
    // Trigger the "afterAllInit" event handlers, from the top class only
    if(!ownerDoc) {
      doc.dispatchEvent(new Event("afterAllInit", {
        propagates: true,
      }));
    }
  }
  ...
  owner() {
    return this._ownerDoc || null;
  }

  rootOwner() {
    return function getUltimateOwner(doc) {
      const owner = doc.owner();

      return owner ? getUltimateOwner(owner) : doc;
    }(this);
  }
